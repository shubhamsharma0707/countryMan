...
class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.hasError)
      return <div className="chart-error">⚠ {this.state.msg || 'Chart failed'}</div>;
    return this.props.children;
  }
}

// ── PlotlyChart — uses window.Plotly (loaded via CDN) ────────────────────────
// No npm import of react-plotly.js at all — bypasses Vite CJS/ESM issues.
function PlotlyChart({ data, layout, height = 380 }) {
  const divRef = useRef(null);

  useEffect(() => {
    if (!divRef.current || !data || !Array.isArray(data)) return;
    if (!window.Plotly) return;          // CDN not loaded yet — skip
    ...
    const mergedLayout = {
      ...
      font: { ...(layout?.font ?? {}), color: '#a1a1aa', family: 'Inter, sans-serif' },
      margin: { l: 10, r: 10, t: 40, b: 30, ...(layout?.margin ?? {}) },
    };

    window.Plotly.newPlot(divRef.current, data, mergedLayout, {
...
function ChartCard({ title, plotData, plotLayout, height = 380 }) {
  if (!plotData || !Array.isArray(plotData) || plotData.length === 0) return null;
  return (
    <motion.div
    ...
    >
      ...
      <ErrorBoundary>
        <PlotlyChart data={plotData} layout={plotLayout} height={height} />
      </ErrorBoundary>
    </motion.div>
  );
}
...
// ── Animated counter ──────────────────────────────────────────────────────────
function Counter({ value, suffix = '', decimals = 1, duration = 1800 }) {
...
function StatCard({ label, value, suffix = '', delta, decimals = 1, icon: Icon }) {
  return (
    ...
    >
      ...
      {delta != null && (
        <div style={{ fontSize: '0.8rem', marginTop: '0.3rem', color: delta > 0 ? '#10b981' : '#f43f5e' }}>
...
export default function App() {
  ...
  const features = [
    ...
    { icon: TrendingUp, title: 'Scenario Engine',       desc: 'Bootstrap confidence intervals and Monte Carlo projections for EDI trajectories.' },
    ...
    <>
      ...
      <div className="metrics-banner" id="dashboard">
        {loading ? (
          ...
          <div className="error-state">⚠ API unreachable — ensure the backend is running on port 8000.<br /><small>{error}</small></div>
        ) : (
      ...
      {data && (
        <section className="section dashboard-section" id="analysis">
          ...
          <div style={{ marginTop: '2rem' }}>
            <ChartCard title="Urban–Rural EDI Surface"        plotData={plots.surface_3d?.data} plotLayout={plots.surface_3d?.layout} height={500} />
          </div>
          ...
          <div style={{ marginTop: '2rem' }}>
            <ChartCard title="State EDI Rankings (Urban)"     plotData={plots.bar?.data}      plotLayout={plots.bar?.layout}
              height={Math.max(460, (data.state_data?.length ?? 30) * 11)} />
          </div>
        </section>
      )}
